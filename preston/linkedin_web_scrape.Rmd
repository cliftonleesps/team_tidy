...
```{r}
extractField <- function(url, field) {
  field <- url %>%
    read_html %>%
    html_element(field) %>%
    html_text2 %>%
...
```{r}
extractEmployment <- function(url, employment) {
  field <- url %>%
    read_html %>%
...
```{r}
extractSalary <- function(url, salary, min = TRUE) {
  field <- extractField(url, salary)
...
for (i in 1:10) {
  ...
  employment <- url %>%
    extractEmployment(employment_type)
    
  description <- url %>%
    extractField(details)
  
  minSalary <- url %>%
    extractSalary(salary_range)
  
  maxSalary <- url %>%
    extractSalary(salary_range, min = FALSE)
...
all_skills_df %>%
  write.csv("all_skills_df_batch_1.csv", row.names = FALSE)
```
